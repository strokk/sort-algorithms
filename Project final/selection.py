...
def selection_sort(A):
	...
	for i in range (0, len(A) - 1):
		#set a min Index
		minIndex = i
		#an inner loop that iterate through the entire unsorted part of the list
		for j in range (i+1, len(A)):
			#Do a comparison to find the min item
			if A[j] < A[minIndex]:
				#save the index of the minimum item
				minIndex = j
			#swap the item into place	
		if minIndex != i:
...
for r in range(nun_runs):
	start_time = time.time()
	selection_sort(array10000.copy())
	end_time = time.time()
	time_elapsed = (end_time - start_time) * 1000
...
for r in range(nun_runs):
	start_time = time.time()
	selection_sort(array12000.copy())
	end_time = time.time()
	time_elapsed = (end_time - start_time) * 1000
